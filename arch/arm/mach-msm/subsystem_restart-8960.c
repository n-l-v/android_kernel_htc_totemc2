...
static int modem_subsystem_restart;
static int modem_ssr_cpu;
static int modem_ssr_delaywork;

static struct workqueue_struct *cpu_wq;
static struct delayed_work cpu_work;

static void __ref cpu_wq_func(struct work_struct *work)
{
	pr_info("cpu_wq_func\n");

	if (cpu_online(1) && modem_ssr_cpu) {
		cpu_down(1);
		modem_ssr_cpu = 0;
	}
	subsys_restart(SUBSYS_AFTER_SHUTDOWN);
	modem_ssr_delaywork = 0;
}

static void __ref subsystem_restart_wq_func(struct work_struct *work)
{
	...
	if (modem_subsystem_restart) {
		if (modem_ssr_delaywork) {
		...
		subsys_restart(SUBSYS_BEFORE_SHUTDOWN);
	}
	...
	if (modem_subsystem_restart) {
		modem_ssr_delaywork = 1;
		queue_delayed_work(cpu_wq, &cpu_work, msecs_to_jiffies(60000));
	}
					...
					struct kobj_attribute *attr, char *buf)
{
	...
	if((get_radio_flag() & 0x8) && !(get_kernel_flag() & KERNEL_FLAG_ENABLE_SSR_MODEM))
	{
...
static int __init subsys_restart_init(void)
{
	...
	INIT_DELAYED_WORK(&cpu_work, cpu_wq_func);

	cpu_wq = alloc_workqueue("cpu_wq", 0, 0);
	if (!cpu_wq)
		pr_err("alloc_workqueue for cpu_wq failed\n");
