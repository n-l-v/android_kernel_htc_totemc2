...
int register_subsys_reboot_notifier(struct notifier_block *nb)
{
	return blocking_notifier_chain_register(&subsys_reboot_notifier_list, nb);
}
EXPORT_SYMBOL(register_subsys_reboot_notifier);

void subsys_restart(unsigned int code)
{
	if (code == MODEM_SUBSYS_BEFORE_SHUTDOWN)
		blocking_notifier_call_chain(&subsys_reboot_notifier_list, MODEM_SUBSYS_BEFORE_SHUTDOWN, NULL);
	else
		blocking_notifier_call_chain(&subsys_reboot_notifier_list, MODEM_SUBSYS_AFTER_SHUTDOWN, NULL);
}
